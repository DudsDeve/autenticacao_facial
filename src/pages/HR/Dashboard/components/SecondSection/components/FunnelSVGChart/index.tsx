...
function getPercent(val:any, prev:any) {
  if (!prev) return "";
...
export function FunnelSVGChart({ data }: { data: any[] }) {
  ...
  useEffect(() => {
    function handleResize() {
      if (containerRef.current) {
        ...
        const height = containerRef.current.offsetHeight || width / aspectRatio;
  ...
  const widths = STEPS_WIDTHS.map((pct) => pct * (numericWidth - margin));
  const [hoverIdx, setHoverIdx] = useState<number | null>(null);
  const [activeStages, setActiveStages] = useState<number[]>([0]);
